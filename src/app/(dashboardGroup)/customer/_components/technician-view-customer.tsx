...
export function TechnicianCustomerCard({ technician }: TechnicianCardProps) {
  // Extract ID gracefully (supporting central type as well as raw backend fallback)
  const techId =
    technician.id ||
  ...
  // Safe cast for legacy/untyped fields on User & Technician objects
  ...
  // Profile Image with fallback (Checks ITechnician first, then raw/legacy user fields)
  const image =
  ...
  // Numeric fields from central type with safe fallbacks
  const hourlyRate =
  ...
  // Format specializations from services array or legacy string/array properties
  ...
  return (
    <div className="group bg-white border border-slate-200 hover:border-blue-400 rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col justify-between">
      ...
      <div className="bg-slate-50 px-5 py-3.5 border-t border-slate-200 flex items-center justify-between">
        ...
        <Link
          href={`/customer/dashboard/technicians/${techId}`}
